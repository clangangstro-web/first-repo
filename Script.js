...
// ========== OB NUMBER MANAGEMENT (Resets Monthly) ==========
...
function visitorIn() {
    ...
    const visitorData = { id: Date.now(), name, phone, idNumber, purpose, host, timeIn, obIn: getNextOB() };
    ...
    saveData();
    renderVisitors();
    alert(`Visitor IN recorded. OB#: ${visitorData.obIn}`);
}
...
function exportToPDF() {
    ...
    printWindow.document.write(`
        <html><head><title>G4S Report</title>
        <style>
        ...
        <table><tr><th>OB#</th><th>Date</th><th>Type</th><th>Person</th><th>Details</th></tr>
...
function exportToCSV() {
    ...
    link.download = `G4S_Report_${new Date().toISOString().slice(0,19)}.csv`;
    link.click();
}
